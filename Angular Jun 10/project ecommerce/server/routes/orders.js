...
router.get("/getAllOrders",function(req,resp){

    con.query("select * from orders",function(err,results){
        if(err){
            console.log(err);
            resp.status(500).send({"message":"orders data loading failed"})
        }
        else{
            resp.send(results);
        }
...
router.get("/getOrderById/:id",function(req,resp){
    con.query("select * from orders where id = ?",[req.params.id],function(err,results){
        if(err){
            console.log(err);
            resp.status(500).send({"message":"orders data loading failed"})
        }
